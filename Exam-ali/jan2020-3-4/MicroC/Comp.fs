...
let ppVar v : string =
  match v with 
  | Locvar x -> "bp[" + (string x) + "]"
  | Glovar x -> "Global[" + (string x) + "]"

// Opgave 3.4
let ppVarTyp (s,(v,t)) : string = 
  s + ":" + (ppTyp t) + " at " + ppVar v

// Opgave 3.5
//type varEnv = (var * typ) env * int
let ppVarEnv (e,_) : string list =
  List.fold (fun acc x -> ppVarTyp x :: acc) [] e

let ppVarEnv' (e,_) =
  List.map ppVarTyp (List.rev e)
